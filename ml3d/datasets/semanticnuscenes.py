...
class SemanticNuScenes(BaseDataset):
    """This class is used to create a dataset based on the SemanticNuScenes
    ...
    def __init__(self,
                 dataset_path,
                 info_path=None,
                 name='SemanticNuScenes',
                 cache_dir='./logs/cache',
                 use_cache=False,
                 class_weights=[
                   46687, 7834, 501416, 410507,
                   4069879, 746905, 696526, 2067585, 1565272
                 ],
    ...
    def read_semseg(path):
        ...
        return np.fromfile(path, dtype=np.uint8).reshape(-1, 1)
    ...
    def get_split(self, split):
        ...
        """
        return SemanticNuScenesSplit(self, split=split)
    ...
    def save_test_result(self, results, data, attr):
        ...
        results = results['predict_labels']
        for res, dat, att in zip([results], data, attr):
            name = att['name']
...
class SemanticNuScenesSplit(BaseDatasetSplit):
    ...
    def get_data(self, idx):
        ...
        labels = self.dataset.read_semseg(semseg_path)
...
DATASET._register_module(SemanticNuScenes)
