...
class mtSalsaNext(BaseModel):
    ...
    def preprocess(self, data, attr):
        ...
        if attr['split'] not in ['test', 'testing']:
            ...
            #yaw = -np.arctan2(y, x) #RV angle is 0 in the left direction
            #    = 0.5 * (-np.arctan2(y, x) / np.pi + 1.0) # in [0.0, 1.0]
            #    = -np.arctan2(y, x) + np.pi               # in [0.0, 2pi]
            #    = -np.arctan2(y, x) - np.pi / 2           # 0 at 3pi / 2
            yaw = -np.arctan2(point[:,1], point[:,0]) - np.pi / 2
            delta_xyz = np.zeros((point.shape[0], 1, 3), dtype=np.float32)
            delta_wlh = np.zeros((point.shape[0], 1, 3), dtype=np.float32)
            delta_r = np.zeros((point.shape[0], 1, 1), dtype=np.float32)
            ...
            for bb in bounding_boxes:
                ...
                delta_wlh[mask] = delta_w, delta_l, delta_h
                d_r = r - yaw
                delta_r[mask] = d_r.reshape(delta_r.shape)[mask]
                diagonal = np.sqrt(w**2 + l**2)
                d_x = (x - point[:,0]) / diagonal
                d_y = (y - point[:,1]) / diagonal
                ang = yaw + np.pi # yaw - (-pi)
                c_a = np.cos(ang)
                s_a = np.sin(ang)
                delta_x =  c_a * d_x + s_a * d_y
                delta_y = -s_a * d_x + c_a * d_y
                delta_z = (z - point[:,2]) / h
                delta_xyz[mask] = np.stack([delta_x, delta_y, delta_z], axis=1).reshape(delta_xyz.shape)[mask]
                #break

            delta = np.concatenate([delta_xyz, delta_wlh, delta_r], axis=-1).squeeze()
    ...
    def inference_end(self, result, input):
        ...
        for _calib, _point, _bbox, _score, _dir in zip(
            ...
            yaw = -torch.arctan2(point[:,:,1], point[:,:,0]) - torch.pi / 2
            p_r = yaw + bbox[6,:,:]
            ang = yaw + torch.pi # yaw - (-pi)
            c_a = torch.cos(ang)
            s_a = torch.sin(ang)
            d_x = c_a * bbox[0,:,:] - s_a * bbox[1,:,:]
            d_y = s_a * bbox[0,:,:] + c_a * bbox[1,:,:]
            diagonal = torch.sqrt(p_w**2 + p_l**2)
            p_x = point[:,:,0] + d_x * diagonal
            p_y = point[:,:,1] + d_y * diagonal
