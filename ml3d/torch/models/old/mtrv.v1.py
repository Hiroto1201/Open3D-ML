...
class mtSalsaNext(BaseModel):
    ...
    def preprocess(self, data, attr):
        ...
        if attr['split'] not in ['test', 'testing']:
            ...
            for bb in bounding_boxes:
                ...
                c_r = np.cos(ang)
                s_r = np.sin(ang)
                delta_x =  c_r * d_x + s_r * d_y
                delta_y = -s_r * d_x + c_r * d_y
                delta_z = (z - point[:,2]) / h
                ...
                d_r = r + np.arctan2(point[:,1], point[:,0]) + np.pi / 2
    ...
    def inference_end(self, result, input):
        ...
        for _calib, _point, _bbox, _score, _dir in zip(
            ...
            d_r = bbox[6,:,:]
            p_r = d_r - torch.arctan2(point[:,:,1], point[:,:,0]) - torch.pi / 2
            ang = p_r + torch.pi
            c_r = torch.cos(ang)
            s_r = torch.sin(ang)
            d_x = c_r * bbox[0,:,:] - s_r * bbox[1,:,:]
            d_y = s_r * bbox[0,:,:] + c_r * bbox[1,:,:]
